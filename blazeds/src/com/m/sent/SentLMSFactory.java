...
public class SentLMSFactory implements SentFactoryAble {
	...
	public BooleanAndDescriptionVO deleteSentGroupList(Connection conn, Connection connSMS, String user_id, int idx, String line, UserInformationVO mvo) {
		...
		int tranResultCount = SLibrary.IfNull(line).equals("ppmms") ?  deleteSentDataOfTranTablePP(connSMS, mvo.getUser_id(), idx) : deleteSentDataOfTranTable(connSMS, mvo.getUser_id(), idx);
	...
	public BooleanAndDescriptionVO cancelSentGroupList(Connection conn, Connection connSMS, UserInformationVO mvo, int idx, String sendLine) throws Exception {
		...
		try {
			...
			if (sentGroupInfo.length == 2 
				...
				int tranResultCount = SLibrary.IfNull(sendLine).equals("ppmms") ?  deleteSentDataOfTranTablePP(connSMS, mvo.getUser_id(), idx) : deleteSentDataOfTranTableKT(connSMS, mvo.getUser_id(), idx);
				...
				int failResultCount = SLibrary.IfNull(sendLine).equals("ppmms") ?selectSentDataOfLogTablePP(connSMS, mvo.getUser_id(), idx) : selectSentDataOfLogTableKT(connSMS, mvo.getUser_id(), idx);
				...
				int bPoint = 0;
				if (sentGroupInfo[2].equals("MM"))
					bPoint = cancelPointPutMM(conn, mvo, tranResultCount +  failResultCount);
				else
					bPoint = cancelPointPut(conn, mvo, tranResultCount +  failResultCount);
	...
	private int selectSentDataOfLogTablePP(Connection conn, String user_id, int idx) {
		...
		PreparedExecuteQueryManager pq = new PreparedExecuteQueryManager();
		pq.setPrepared(conn,VbyP.getSQL("selectLMSSentDataLogtablePP") );
		pq.setString(1, user_id);
		pq.setString(2, Integer.toString(idx) );
		return pq.ExecuteQueryNum();
	}
	...
	private int cancelPointPutMM(Connection conn, UserInformationVO mvo, int cnt) {
		
		PointManager pm = PointManager.getInstance();
		
		return pm.insertUserPoint(conn, mvo, 26, cnt * PointManager.DEFULT_POINT*15);
	}
} 
